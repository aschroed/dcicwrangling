...
from src.scripts import generate_wfr_from_pf as gw
...
def test_create_wfr_meta_only_json_w_no_wf_uuid(mocker, auth, prov_workflow):
    ...
    mocker.patch('src.scripts.generate_wfr_from_pf.scu.get_item_if_you_can', return_value=prov_workflow)
...
def test_create_wfr_meta_only_json_w_alias_and_desc(mocker, auth, prov_workflow, infiles, outfile):
    ...
    mocker.patch('src.scripts.generate_wfr_from_pf.scu.get_item_if_you_can',
                 side_effect=[prov_workflow, infiles[0], infiles[1], outfile[0]])
    mocker.patch('src.scripts.generate_wfr_from_pf.datetime', return_value='2018-06-11 16:29:22.839062')
...
def test_wfr_main_no_auth(mocker, capsys, mocked_args_no_args):
    with pytest.raises(SystemExit):
        mocker.patch('src.scripts.generate_wfr_from_pf.get_args', return_value=mocked_args_no_args)
        gw.main()
        out = capsys.readouterr()[0]
...
def test_wfr_main_no_parents(mocker, capsys, mocked_args_dbupd_is_false, auth, prov_workflow,
    ...
    mocker.patch('src.scripts.generate_wfr_from_pf.get_args', return_value=mocked_args_dbupd_is_false)
    mocker.patch('src.scripts.generate_wfr_from_pf.get_authentication_with_server', return_value=auth)
    mocker.patch('src.scripts.generate_wfr_from_pf.scu.get_item_ids_from_args', return_value=[fp_data['uuid']])
    mocker.patch('src.scripts.generate_wfr_from_pf.get_metadata', side_effect=[prov_workflow, fp_data])
    gw.main()
    out = capsys.readouterr()[0]
    assert not out
...
def test_wfr_main_dryrun(mocker, capsys, mocked_args_dbupd_is_false, auth, prov_workflow,
    ...
    mocker.patch('src.scripts.generate_wfr_from_pf.get_authentication_with_server', return_value=auth)
    mocker.patch('src.scripts.generate_wfr_from_pf.scu.get_item_ids_from_args', return_value=[fp_data['uuid']])
    mocker.patch('src.scripts.generate_wfr_from_pf.get_metadata',
                 side_effect=[prov_workflow, fp_data, infiles[0], infiles[1]])
    mocker.patch('src.scripts.generate_wfr_from_pf.create_wfr_meta_only_json', return_value=wfr_out_json)
    gw.main()
    out = capsys.readouterr()[0]
...
def test_wfr_main_dbupdate(mocker, capsys, mocked_args_dbupd, auth,
    ...
    mocker.patch('src.scripts.generate_wfr_from_pf.get_args', return_value=mocked_args_dbupd)
    mocker.patch('src.scripts.generate_wfr_from_pf.get_authentication_with_server', return_value=auth)
    mocker.patch('src.scripts.generate_wfr_from_pf.scu.get_item_ids_from_args', return_value=[fp_data['uuid']])
    mocker.patch('src.scripts.generate_wfr_from_pf.get_metadata',
                 side_effect=[prov_workflow, fp_data, infiles[0], infiles[1]])
    mocker.patch('src.scripts.generate_wfr_from_pf.create_wfr_meta_only_json', return_value=wfr_out_json)
    mocker.patch('src.scripts.generate_wfr_from_pf.post_metadata', return_value='SUCCESS')
    gw.main()
    out = capsys.readouterr()[0]
